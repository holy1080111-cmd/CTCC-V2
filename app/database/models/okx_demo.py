...
class OkxDemoBalanceState(Base):
    __tablename__ = "okx_demo_balance_state"
...
class OkxDemoOrderState(Base):
    __tablename__ = "okx_demo_order_state"

    order_id: Mapped[str] = mapped_column(String(100), primary_key=True)
    client_order_id: Mapped[str | None] = mapped_column(String(32), unique=True)
...
class OkxDemoPositionState(Base):
    __tablename__ = "okx_demo_position_state"

    position_key: Mapped[str] = mapped_column(String(80), primary_key=True)
    instrument_id: Mapped[str] = mapped_column(String(40), nullable=False, index=True)
...
class OkxDemoAlgoOrderState(Base):
    __tablename__ = "okx_demo_algo_order_state"

    algo_order_id: Mapped[str] = mapped_column(String(100), primary_key=True)
    client_algo_order_id: Mapped[str | None] = mapped_column(String(32), unique=True)
    instrument_id: Mapped[str] = mapped_column(String(40), nullable=False, index=True)
    order_type: Mapped[str] = mapped_column(String(32), nullable=False)
    state: Mapped[str] = mapped_column(String(32), nullable=False, index=True)
...
class OkxDemoSyncCheckpoint(Base):
    __tablename__ = "okx_demo_sync_checkpoints"
    __table_args__ = (CheckConstraint("id = 1", name="singleton_id"),)
