...
def analyze_timeframe(
    timeframe: str, candles: list[Candle], quality
) -> TimeframeAnalysis:
    ...
    atr_pct = (
        None
        if atr14 is None or confirmed[-1].close <= 0
        else atr14 / confirmed[-1].close * D("100")
    )
    ...
    indicators = IndicatorSnapshot(
        ...
        atr14=_q(atr14),
        atr_pct=_q(atr_pct),
        rsi14=_q(rsi(closes, 14)),
        macd=_q(macd_value),
        macd_signal=_q(macd_signal),
        macd_histogram=_q(macd_hist),
        adx14=_q(adx(confirmed, 14)),
        vwap=_q(vwap(confirmed, 20)),
        volume_ratio20=_q(volume_ratio(confirmed, 20)),
        causal_trend=(
            CausalTrendSnapshot(
                window=trend.window,
                log_velocity_per_bar=_q(trend.log_velocity_per_bar, "0.000000000001"),
                ...
                acceleration_to_volatility=_q(trend.acceleration_to_volatility),
        ...
        causal_state=(
            CausalStateSnapshot(
                window=state.window,
                log_velocity_per_bar=_q(state.log_velocity_per_bar, "0.000000000001"),
                log_acceleration_per_bar2=_q(
                    state.log_acceleration_per_bar2, "0.000000000001"
                ),
                velocity_std=_q(state.velocity_std, "0.000000000001"),
                acceleration_std=_q(state.acceleration_std, "0.000000000001"),
        ...
        return_interval=(
            CausalReturnIntervalSnapshot(
                ...
                half_width=_q(return_interval.half_width, "0.000000000001"),
    ...
    if structure.trend in {"strong_bullish", "bullish"}:
        evidence.append("EMA trend supports long bias")
    elif structure.trend in {"strong_bearish", "bearish"}:
        evidence.append("EMA trend supports short bias")
    if structure.bos:
        evidence.append(f"confirmed close is beyond latest swing: BOS {structure.bos}")
    if indicators.macd_histogram is not None:
        (evidence if indicators.macd_histogram > 0 else counter).append(
            "MACD histogram is positive"
            if indicators.macd_histogram > 0
            else "MACD histogram is negative"
        )
    if indicators.rsi14 is not None and indicators.rsi14 >= D("70"):
        counter.append("RSI is overbought")
    if indicators.rsi14 is not None and indicators.rsi14 <= D("30"):
        counter.append("RSI is oversold")
    if indicators.volume_ratio20 is not None and indicators.volume_ratio20 < D("0.8"):
        counter.append("latest volume is below its 20-candle baseline")
    ...
    if structure.trend in {"strong_bullish", "bullish"} and structure.choch != "down":
        bias = "long"
    elif structure.trend in {"strong_bearish", "bearish"} and structure.choch != "up":
        bias = "short"
    else:
        bias = "neutral"
    ...
    return TimeframeAnalysis(
        ...
        data_quality_issues=issue_strings,
...
class AnalysisService:
    ...
    async def analyze(
        self, symbol: str, candle_limit: int = 250
    ) -> MultiTimeframeAnalysis:
    ...
    def analyze_snapshot(self, snapshot: MarketSnapshot) -> MultiTimeframeAnalysis:
        views = {
            bar: analyze_timeframe(bar, snapshot.candles[bar], snapshot.quality[bar])
            for bar in snapshot.candles
        }
        return _combine_analysis(
            snapshot,
            views,
            snapshot.quality,
...
def _combine_analysis(snapshot, views, quality, *, evaluated_at, version):
    mathematical_core = mathematical_core_snapshot(views)
    blockers: list[str] = []
    for bar, report in quality.items():
        if not report.ok:
            blockers.append(f"{bar}_data_quality")
    ordered = [
        views[bar].directional_bias for bar in ("4H", "1H", "15m", "5m") if bar in views
    ]
    ...
    elif (overall == "long" and mathematical_core.status == "short") or (
...
def analyze_snapshot_at(
    snapshot: MarketSnapshot, *, evaluated_at: datetime, version: str
) -> MultiTimeframeAnalysis:
    """Recompute descriptive analysis with explicit clocks, version and quality.

    No service/client construction, wall clock or settings access occurs here.
    This is not a complete source-authenticity or trade-qualification gate; the
    caller must separately validate the bounded snapshot's other components.
    """
    if type(snapshot) is not MarketSnapshot:
        raise ValueError("an exact MarketSnapshot is required")
    evaluated = _utc_time(evaluated_at)
    received = _utc_time(snapshot.received_at)
    if received > evaluated:
        raise ValueError("snapshot was captured after evaluation")
    if (
        ...
        raise ValueError(
            "an explicit nonempty version of at most 64 characters is required"
        )
    if type(snapshot.candles) is not dict or set(snapshot.candles) != set(BAR_SECONDS):
        raise ValueError("exactly four supported candle timeframes are required")
    if (
        type(snapshot.ticker.last) is not Decimal
        or not snapshot.ticker.last.is_finite()
        or snapshot.ticker.last <= 0
    ):
        raise ValueError("snapshot price must be a positive finite Decimal")
    views = {}
    quality = {}
    context = Context(
        prec=100,
    ...
    with localcontext(context):
        for bar in ("4H", "1H", "15m", "5m"):
            rows = snapshot.candles[bar]
            report = inspect_candles_at(rows, bar, current_time=evaluated)
            unsafe = [
                issue.code
                for issue in report.issues
                if issue.code not in {"STALE_CANDLE", "UNCONFIRMED_CANDLE"}
            ]
            if unsafe:
                raise ValueError(f"{bar}: {','.join(unsafe)}")
            normalized = []
            for row in rows:
                copied = row.model_copy(update={"timestamp": _utc_time(row.timestamp)})
                if copied.confirmed and candle_closed_at(copied, bar) > received:
                    raise ValueError(f"{bar}: confirmed candle closed after capture")
                normalized.append(copied)
            quality[bar] = report
            views[bar] = analyze_timeframe(bar, normalized, report)
        return _combine_analysis(
            snapshot,
            views,
            quality,
