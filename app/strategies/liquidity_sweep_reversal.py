...
def _sweep(ctx: StrategyContext, direction: str) -> bool:
    ...
    if direction == "long" and view.structure.last_swing_low is not None:
        return (
            view.close > view.structure.last_swing_low and view.structure.choch == "up"
        )
    if direction == "short" and view.structure.last_swing_high is not None:
        return (
            view.close < view.structure.last_swing_high
            and view.structure.choch == "down"
        )
    return False
...
def conditions(ctx: StrategyContext) -> StrategyConditionSet:
    direction = (
        ...
        if ctx.tf("15m").structure.choch == "down"
        else "neutral"
    )
    conditions = [
        Condition(
            "15m_sweep",
            "15m liquidity sweep is reclaimed",
            30,
            _sweep(ctx, direction),
            "no reclaim after a liquidity sweep",
            required=True,
        ),
        Condition(
            "15m_choch",
            "15m CHoCH confirms reversal",
            25,
            choch_matches(ctx.tf("15m"), direction),
            "15m CHoCH is missing",
            required=True,
        ),
        Condition(
            "5m_momentum",
            "5m momentum confirms reversal",
            20,
            momentum_matches(ctx.tf("5m"), direction),
            "5m momentum has not confirmed",
            required=True,
        ),
        Condition(
            "not_extreme",
