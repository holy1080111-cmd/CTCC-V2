...
def evaluate(ctx: StrategyContext):
    ...
    conditions = [
        Condition("4h_direction", "4H direction agrees", 20, trend_matches(ctx.tf("4H"), direction), "4H direction disagrees", required=True),
        Condition("1h_context", "1H direction agrees", 15, trend_matches(ctx.tf("1H"), direction), "1H context disagrees"),
        Condition("15m_order_block", "15m price is retesting an unmitigated directional order block", 35, _at_directional_block(ctx, direction), "price is not retesting a valid 15m order block", required=True),
        Condition("5m_trigger", "5m momentum confirms the return", 15, momentum_matches(ctx.tf("5m"), direction), "5m trigger is missing", required=True),
        Condition("quality", "Data quality passes", 15, all(v.data_quality_ok for v in ctx.analysis.timeframe_analyses.values()), "data quality failed", veto=True, required=True),
    ]
