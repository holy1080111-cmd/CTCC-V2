...
def evaluate(ctx: StrategyContext):
    ...
    conditions = [
        ...
        Condition(
            ...
            "1H context disagrees",
        ),
        Condition(
            "15m_order_block",
            "15m price is retesting an unmitigated directional order block",
            35,
            _at_directional_block(ctx, direction),
            "price is not retesting a valid 15m order block",
            required=True,
        ),
        Condition(
            "5m_trigger",
            "5m momentum confirms the return",
            15,
