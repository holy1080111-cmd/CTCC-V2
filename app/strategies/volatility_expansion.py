...
def evaluate(ctx: StrategyContext):
    ...
    conditions = [
        Condition("15m_expansion", "15m volatility expands", 25, ctx.tf("15m").volatility == "high", "15m volatility is not in controlled expansion", required=True),
        Condition("15m_bos", "15m BOS confirms expansion direction", 25, bos_matches(ctx.tf("15m"), direction), "15m BOS is missing", required=True),
        ...
        Condition("5m_momentum", "5m momentum agrees", 15, momentum_matches(ctx.tf("5m"), direction), "5m momentum disagrees", required=True),
        Condition("not_extreme", "5m volatility is not extreme", 15, ctx.tf("5m").volatility != "extreme", "5m volatility is extreme", veto=True),
        Condition(
            "quality", "All required timeframes and analysis quality pass", 0,
            not ctx.analysis.blockers and all(
