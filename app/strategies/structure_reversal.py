...
def evaluate(ctx: StrategyContext):
    ...
    conditions = [
        Condition("1h_choch", "1H CHoCH confirms structural reversal", 35, choch_matches(ctx.tf("1H"), direction), "1H CHoCH is missing", required=True),
        Condition("15m_follow", "15m bias follows reversal", 25, ctx.tf("15m").directional_bias == direction, "15m does not follow reversal", required=True),
        Condition("5m_trigger", "5m momentum confirms entry", 20, momentum_matches(ctx.tf("5m"), direction), "5m trigger is missing", required=True),
