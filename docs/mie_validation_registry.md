...
| MIE Gate 3 offline validation foundation | computational contracts, synthetic leakage verification, fail-closed real-batch qualification, and prospective seal/receipt state machine | offline fixtures plus hash-bound public reference data | shadow-only calculation tooling | row-availability qualification and adapter, real frozen candidate/seal, seal/receipt-to-evidence link, fresh unread holdout replay, and independent OOS review |
...
| MIE daily archive availability and rehearsal | computational; original ZIP membership and conservative retrieval-time conversion | caller-supplied archive bytes and observation attestation | offline development/validation rehearsal only | independently qualified timestamps, reviewed split-plan linkage, real acquisition chain, batch integration and prospective OOS review |
