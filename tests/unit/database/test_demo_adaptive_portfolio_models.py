...
def test_demo_automation_state_persists_portfolio_and_symbol_cooldowns() -> None:
    ...
    assert isinstance(table.c.realized_pnl_events.type, JSONB)
    assert table.c.realized_pnl_events.nullable is False
    assert table.c.risk_peak_equity.nullable is True
